...
public class StarRatingCheckboxesTest {
    static final Logger logger = LogManager.getLogger(StarRatingCheckboxesTest.class);
    ...
    @Parameters({"HOTEL_FILTER_PAGE_INDEX", "WORKFLOW_USER_INDEX", "TRIP-STAR_RATING_FIRST", "TRIP-STAR_RATING_SECOND"})
    public void setup(int hotelFilterPageIndex, int userIndex, String starRatingFirst, String starRatingSecond) {
        UserList.getInstance().getUsers().get(userIndex).getTrip().getHotelStarRating().add(starRatingFirst);
        UserList.getInstance().getUsers().get(userIndex).getTrip().getHotelStarRating().add(starRatingSecond);
    ...
    @Test(groups = {"choosehotel"}, dependsOnGroups = {"hotelFiltering"})
    ...
    public void verifyCheckboxesMatching(int userIndex) throws UnsupportedDriverNameException, MissingFileException {
        hotelFilterPage.selectFilters(UserList.getInstance().getUsers().get(userIndex), "stars");
    ...
    public void teardown() throws UnsupportedDriverNameException, MissingFileException {
        hotelFilterPage.showAvailable();
        PageList.getInstance().getPages().add(hotelFilterPage.chooseHotel());
